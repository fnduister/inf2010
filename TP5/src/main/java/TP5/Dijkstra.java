...
//---------------------------------------------------------------------------
		 ...
		 dijkstraTable[0]=map;
		 Node[] noeudtraite =new Node[graph.getNodes().size()] ;
		 ...
		 for(int i=1;i<graph.getNodes().size();i++) {
			 ...
			 //extraire les noeuds qui font office de destination a cet arc et n'ayant pas ecorre ete traite
			...
			//	 ajouter le noeud s'il n'a pas encore ete traite
			 ...
			 //---------------------------------------------------------------------
			 //ajouter les  noeuds non traiter de l'ancienne itération qui n'ont pas ete insérer dans l'itération présente
			 ...
			 //comparer les valeurs précedentes avec les valeurs actuelles et mettre a jour la table
			  ...
			  for(int l=0;l<graph.getNodes().size();l++) {
				...
				//vérifier que le noeud n'a pas encore ete traite
				  if(dejatraite==false) {
						Edge edge_minimal=getMinimum(dijkstraTable[i].get(graph.getNodes().get(l)), dijkstraTable[i-1].get(graph.getNodes().get(l))) ;
			...
			/* for(Edge edge :list_edge) {
				 ...
				 if(edge.getDestination()==noeud_courant) {
					 chercher=edge;
					 break;
		    ...
		    for(Map.Entry<Node,Edge> entry : dijkstraTable[i].entrySet()) {
			 ...
			 distance_minimale=chercher.getDistance();// la nouvelle distance est entre le noeud courant  et le noeud de depart
...
//--------------------------------------------------------------------------------
	...
	public void afficherTable() {
		...
		for(int i=0;i<dijkstraTable.length;i++) {
			...
			System.out.print("Itération"+i +":");
		   for(Map.Entry<Node,Edge> entry : dijkstraTable[i].entrySet()) {
			
			   System.out.println("Le noeud "+entry.getKey().getName() +" a pour distance " + entry.getValue().getDistance() +" et pour source " +entry.getValue().getSource().getName());
